...
metadata_dir = results_dir / "metadata.txt"
...
for prompt in prompts:
    ...
    for i in range(len(model.model.layers)):
        result += f"Quantized layer {i}:\n"
        model_q = AutoModelForCausalLM.from_pretrained(
            model_name,
            torch_dtype=original_type
        )
        model_q = model_q.to(device)
        quantize(model_q.model.layers[i])
        outputs_q = model_q(
            **inputs,
            output_hidden_states=True
        )

        for j, (fp, q) in enumerate(zip(baseline_hidden, outputs_q.hidden_states)):
            d = damage(fp, q)
            ...
            result += f"{j:02d}; MSE: {d:.4f}; cos: {cos:.4f}\n"
...
plt.savefig(
    results_dir / "heatmap_mse.png",
    bbox_inches="tight"
)

for i, (prompt, result) in enumerate(zip(prompts, prompt_results)):
    with open(prompts_dir / f"prompt{i}.txt", "w") as f:
        f.write(prompt + "\n" + result + "\n")
...
for prompt in df["prompt"].unique():
    for layer in df["quantized_layer"].unique():
        ...
        plt.grid(True)
        plt.savefig(results_dir / f"layer{layer:02d}.png", bbox_inches="tight")
